const validateUser = (req, res, next) => {
  ...
  if (!newEmail) {
    ...
    if (!regexEmail.test(newEmail)) {
      errors.push("Este e-mail está inválido");
    } else if (newEmail.length > 150) {
      errors.push("O e-mail deve ter no máximo 150 caracteres!");
    }
  }
  ...
  if (!newPassword) {
    ...
    errors.push(
      "A senha deve ter pelo menos 8 caracteres, contendo uma letra maiúscula, e minúscula, número e caractere especial"
    );
  }
